...
namespace igx.jobs.fusionruleprocessor
{
    ...
    {
        public FusionReferenceItemPromotionAction(FusionRuleStepModel step, TextWriter log, int companyId, FusionRule rule, int promoteToObjectID, string promoteToObject)
        {
            Step = step;
            CompanyId = companyId;
            Rule = rule;
        ...
        {            
            ...
            if (codeTarget == null)
            {
                Log.WriteLine($"Company ID[{CompanyId}] Rule ID[{Rule.ID}] Step ID[{Step.ID}] Action[{Step.Action}] Doesnt contain a mapping for Code field.");
