...
namespace igx.jobs.fusionruleprocessor
{
    ...
    {
        public FusionRuleFusionFind(FusionRuleStepModel step, TextWriter log, int companyId, FusionRule rule)
        {
            Step = step;
            Log = log;
            CompanyId = companyId;
        ...
        public async Task Execute(List<int> itemsToPromote, SqlConnection company)
        {
            ...
            {
                Log.WriteLine($"Company ID[{CompanyId}] Rule ID[{Rule.ID}] Step ID[{Step.ID}] Action Find Fusion Unknown Fusion find object type.");
        ...
        private async Task FindFusionAttribute(List<int> itemsToPromote, SqlConnection company)
        {
            ...
            await company.ExecuteAsync(sql, new { RuleID = Rule.ID, RuleStepID = Step.ID,  FindFilterFieldID, items = itemsToPromote });
        }
