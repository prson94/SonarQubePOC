...
namespace igx.jobs.fusionruleprocessor
{
    ...
    {
        public FusionRuleResultFromStepFind(FusionRuleStepModel step, TextWriter log, int companyId, FusionRule rule)
        {
            Step = step;
            Log = log;
            CompanyId = companyId;
            ...
            using (var transaction = company.BeginTransaction())
            {
                ...
                if (FindParent > 0)
                {
                    ...
                    WHEN NOT MATCHED BY SOURCE AND T.RuleID = @RuleID and T.RuleStepID = @RuleStepID
                        THEN DELETE;
                    ", new { RuleID = Rule.ID, FindSearchObjectID, AttributeType = Rule.ObjectType.Replace("Type", ""), RuleStepID = Step.ID }, transaction:transaction);
                }
                else
                {
