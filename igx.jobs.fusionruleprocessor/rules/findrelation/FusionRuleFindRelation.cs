...
namespace igx.jobs.fusionruleprocessor
{
    ...
    {
        public FusionRuleFindRelation(FusionRuleStepModel step, TextWriter log, int companyId, FusionRule rule)
        {
            Step = step;
            Log = log;
            CompanyId = companyId;
        ...
        {               
            ...
            {
                Log.WriteLine($"Company ID[{CompanyId}] Rule ID[{Rule.ID}] Step ID[{Step.ID}] Find Relation Step doesnt have an intersectTypeID specified.");
            ...
            {
                Log.WriteLine($"Company ID[{CompanyId}] Rule ID[{Rule.ID}] Step ID[{Step.ID}] Find Relation Step doesnt have a search type specified.");
        ...
        private async Task LoadItemsForFind(string searchType, List<int> itemsToPromote, SqlConnection company)
        {
            ...
            else if(string.Compare(searchType, "ResultFromStep", true) == 0)
            {
                ...
                {
                    Log.WriteLine($"Company ID[{CompanyId}] Rule ID[{Rule.ID}] Step ID[{Step.ID}] Find Relation Step is of type ResultFromStep but doesnt specify a step id.");
