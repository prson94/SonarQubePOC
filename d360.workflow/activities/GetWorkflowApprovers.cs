...
{
    public sealed class GetWorkflowApprovers : CodeActivity
    {
        ...
        protected override void Execute(CodeActivityContext context)
        {
            ...
            using (var cnn = CompanyConnectionUtils.GetCompanyConnection(companyID))
            {
                ...
                var owners = cnn.Query<Resource>(@"exec utility.GetApproversForWorkflow @w", new { w = context.WorkflowInstanceId }).ToList();
            ...
            var trackingRecord = new CustomTrackingRecord(context.WorkflowInstanceId, "Get Artifact Approvers", System.Diagnostics.TraceLevel.Info)
            {
                Data =
                        {
                            {"CompanyID", companyID},
