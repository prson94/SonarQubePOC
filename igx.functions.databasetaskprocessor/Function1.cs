    ...
    public static class DatabaseTaskProcessor
    {
        ...
        public static void Run([TimerTrigger(timerSettings, RunOnStartup = true)]TimerInfo myTimer, System.IO.TextWriter log)
        {
            try
            {
                ...
                System.Threading.Thread.Sleep(sleepSeconds * 500);
                #endregion
...
";
        ...
        private static void addAuditEntry(System.Data.SqlClient.SqlConnection companyConnection, string @object, int objectID, string oper, string custom, long assetID)
        {
            if (!string.IsNullOrEmpty(custom))
            {
                var customXml = System.Xml.Linq.XElement.Parse(custom);
