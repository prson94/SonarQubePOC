...
{
    public class ResponsibilitiesController_DeleteResponsibilityRules_Tests
    {
        [Theory, AutoData]
        ...
        )
        {
            ...
            mockMediator.Setup(x => x.Send(It.Is<ResponsibilityDeleteRulesRequest>(r => r.TypeUid == typeUid && r.RuleDeleteUidCollection.All(expectedUids.Contains)), default)).ReturnsAsync(businessLayerResponse);
            ...
            mockMediator.Verify(x => x.Send(It.Is<ResponsibilityDeleteRulesRequest>(r => r.TypeUid == typeUid && r.RuleDeleteUidCollection.All(expectedUids.Contains)), default), Times.Once);
        }
    }
}