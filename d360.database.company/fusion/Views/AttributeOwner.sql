...
as
	select	R.FusionID,
			...
			R.ObjectID,
			R.ObjectType,
			OBJ.Name + ISNULL( ' (' + OBJ.ObjectTypeName + ')','') as ObjectName,
			R.ParentObjectID,
			R.ParentObjectType,
			PAR.Name + ISNULL( ' (' + PAR.ObjectTypeName + ')','')  as ParentName,
			R.RelationshipOwnerObjectID,
			R.RelationshipOwnerObjectType,
			REL.Name +  ISNULL(' (' + REL.ObjectTypeName + ')','') as RelationshipOwnerName
