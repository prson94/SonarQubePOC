...
begin
	...
	if @Object = 'PolicyType' OR @Object = 'Policy'
	begin
		---------------------------------------------------
	...
	if @Object = 'RuleType' OR @Object = 'Rule'
	begin
	...
	if @Object = 'TaxonomyType' OR @Object = 'Taxonomy' OR @Object = 'ArtifactType' OR @Object = 'Artifact'
	begin
		...
		insert into #Responsibilities
			...
			where	(@Object = 'Artifact' and exists(select 1 from Artifact where ID = @ObjectID and TaxonomyTypeID = T.ID))
					OR (@Object = 'ArtifactType' and exists(select 1 from Artifact where ArtifactTypeID = @ObjectID and TaxonomyTypeID = T.ID))
					OR (@Object = 'Taxonomy' and exists(select 1 from Taxonomy where ID = @ObjectID and TaxonomyTypeID = T.ID))
					OR (@Object = 'TaxonomyType' and T.ID = @ObjectID);
		---------------------------------------------------
		...
		begin
			insert into #Responsibilities
				...
				from	TaxonomyType T 
						...
						inner join Artifact AR on AR.ID = @ObjectID and AR.TaxonomyTypeID = T.ID
						inner join cache.Relationships RE on RE.SourceObject = 'Taxonomy' and RE.SourceObjectID = A.ID and RE.TargetObject = 'Artifact' and RE.TargetObjectID = AR.ID;
		end
		...
		begin
			insert into #Responsibilities
				...
				from	TaxonomyType T 
						inner join Responsibility R on R.ObjectType = 'TaxonomyType' and R.ObjectID = T.ID
						inner join Taxonomy A on A.TaxonomyTypeID = T.ID
						inner join Artifact AR on AR.ArtifactTypeID = @ObjectID and AR.TaxonomyTypeID = T.ID
						inner join cache.Relationships RE on RE.SourceObject = 'Taxonomy' and RE.SourceObjectID = A.ID and RE.TargetObject = 'Artifact' and RE.TargetObjectID = AR.ID;
		end
		if @Object = 'Taxonomy'
		begin
			insert into #Responsibilities
				...
				from	TaxonomyType T 
						inner join Responsibility R on R.ObjectType = 'TaxonomyType' and R.ObjectID = T.ID
						inner join Taxonomy A on A.TaxonomyTypeID = T.ID and A.ID = @ObjectID;
		end
		...
		begin
			insert into #Responsibilities
				...
				from	TaxonomyType T 
						inner join Responsibility R on R.ObjectType = 'TaxonomyType' and R.ObjectID = T.ID
						inner join Taxonomy A on A.TaxonomyTypeID = T.ID and T.ID = @ObjectID;
		end;
		-------------------------------------------------
		...
		begin
			...
			begin
				...
				insert into #Responsibilities
					select	'Artifact Type Direct' as [Source],
							...
							1 as [Priority]
					from	ArtifactType T 
							inner join Responsibility R on R.ObjectType = 'ArtifactType' and R.ObjectID = T.ID and T.ID = @ObjectID;
				---------------------------------------------------
			end
			...
			begin
				insert into #Responsibilities
					...
					from	ArtifactType T 
							...
							inner join Artifact A on A.ArtifactTypeID = T.ID and A.ArtifactTypeID = @ObjectID;
			end
			if @Object = 'Artifact'
			begin
				insert into #Responsibilities
					...
					from	ArtifactType T 
							inner join Responsibility R on R.ObjectType = 'ArtifactType' and R.ObjectID = T.ID
							inner join Artifact A on A.ArtifactTypeID = T.ID and A.ID = @ObjectID;
			end
			-------------------------------------------------
			...
			insert into #Responsibilities
				select	'Artifact Override' as [Source],
						...
						3 as [Priority]
				from	Artifact T 
						inner join Responsibility R on R.ObjectType = 'Artifact' and R.ObjectID = T.ID and @Object = 'Artifact' and T.ID = @ObjectID;
			-------------------------------------------------
		end --IF CONDITION: ARTIFACT/TYPE

	end --IF CONDITION: TAXONOMY/TYPE OR ARTIFACT/TYPE
	...
	where	ID not in
			...
			from	#Responsibilities T
					inner join	(
								...
								group by ResponsibilityTypeID, [Object], ObjectID, ContextHash
								) S on S.ResponsibilityTypeID = T.ResponsibilityTypeID 
									and S.[Object] = T.[Object] and S.[ObjectID] = T.[ObjectID] 
									and S.[ContextHash] = T.[ContextHash] and S.[Priority] = T.[Priority]
			)

	delete cache.ResponsibilityItem where [Object] = @Object and ObjectID = @ObjectID

	insert into cache.ResponsibilityItem--ies
	(
