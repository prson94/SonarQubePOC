...
begin
	...
	begin try
		...
		insert into [queue].[ObjectVersion] 
			([Object], ObjectID, ResourceID, [Date], [Action], ActionObject, ActionObjectID)
		values 
			(@Object, @ObjectID, @ResourceID, getutcdate(), 'Removed', @Object, @ObjectID)

		insert into [queue].[ObjectIndex] 
			([Object], ObjectID, ResourceID, [Date], [Action], ActionObject, ActionObjectID)
		values
			(@Object, @ObjectID, @ResourceID, getutcdate(), 'D', @Object, @ObjectID)
		...
		begin
			...
			begin
				...
				delete Field							where FieldTypeID in (select ID from FieldType where [Object] = @Object AND ObjectID = @ObjectID)
		...
		if @Object = 'RuleType'
		begin
			delete [Rule] where RuleType = @ObjectID
		end

		if @Object = 'QuestionType'
		begin
			delete Question where QuestionTypeID = @ObjectID
		end
