...
--update UID root row, set ParentID to row with UID rootArtifact## being the object+id from the root row
--delete the rootArtifact## row

delete from @results where [UID] = 'root' and ID not in (select top 1 ID from @results where [UID] = 'root');

update r
set r.ParentID = r2.ParentID
from @results r
join @results r2 on r2.[UID] like 'root' + r.[object] + '%'
where r.[UID] = 'root';

delete from @results 
where [UID] like (select 'root' + [object] + '%'
				from @results
				where [UID] = 'root');  
...
begin
	select * from @results order by [level] asc;
end
