...
AS
	SELECT	T.ID as FieldTypeID,
			T.LookupObjectType,
			T.LookupObjectID,
			COALESCE(A.ID, D.ID, DI.ID, R.ResourceID, L.ID, RI.ID) as Value,			
			utility.GetFormattedFieldLookupValue(T.Type, T.LookupDisplayFormat, T.LookupObjectType, T.LookupObjectID, COALESCE(A.ID, D.ID, DI.ID, R.ResourceID, L.ID, RI.ID)) AS Text
	FROM	FieldType T 
	...
	WHERE	T.LookupObjectType is not null
			AND COALESCE(A.ID, D.ID, DI.ID, R.ResourceID, L.ID,RI.ID) IS NOT NULL
