CREATE procedure [tile].[GetRedFlagsByTypeAndResource]
--declare
	@type varchar(50),
	@id int,
	@resourceID int
--set @type = 'ArtifactType'
--set @id = 1
--set @resourceID = 1
as
begin
	with f as	(
				select	ObjectType, 
						ObjectID 
				from	Follow 
				where	ResourceID = @resourceID				
				),
		rg as	(
				select	ObjectType,
						ObjectID
				from	ResponsibilityDetail rd
						inner join ResourceGroup rg on rd.ResponsibleObjectType = 'Group' and rg.GroupID = rd.ResponsibleObjectID and rg.ResourceID = @resourceID
				),
		r as	(
				select	ObjectType,
						ObjectID
				from	ResponsibilityDetail
				where	ResponsibleObjectType = 'Resource'
						and ResponsibleObjectID = @resourceID
				)

	select		FD.ObjectID as ID,
				FD.Name,
				FD.TextPath,
				FD.Description,
				FD.ParentID,
				FD.Parent as ParentType,
				FD.Url,
				FD.ObjectTypeID as TypeID,
				FD.ObjectType as [Type],
				FD.ObjectTypeName as [TypeName],
				FD.IconBackColor,
				FD.IconForeColor,
				FD.IconText,
				CR.[Count] as CriticalRelationshipCount
	from		(
					select	* from f
					union
					select	* from r
					union
					select	* from rg
				) F
				inner join AlertFlag A on A.ObjectType = F.ObjectType and A.ObjectID = F.ObjectID and A.Active = 1
				inner join cache.ObjectDetails FD on FD.[Object] = F.ObjectType and FD.ObjectID = F.ObjectID and FD.ObjectType = @type and FD.ObjectTypeID = @id
				--cross apply utility.ObjectDetail(F.ObjectType, F.ObjectID) FD
				outer apply (
							select		count(1) as [Count] 
							from		IntersectNode S
										inner join [Intersect] I on S.ObjectType = F.ObjectType 
																	and S.ObjectID = F.ObjectID
																	and I.ID = S.IntersectID 
																	and I.Classification = 1
							) CR
end
