...
{TAG_API_MODEL_SQL_WITHOUT_WHERE} where t.ID = @tagId;",
		...
		public async Task<RepositoryResponse<TagTypeApiModel>> CreateTagTypeAsync(string value)
		{
			...
			if (!value.IsValidForTag(out bool isMaxLengthExceeded))
			{
				...
				{
					return new RepositoryResponse<TagTypeApiModel>(null, 400, false, string.Format(Error.MaxLengthExceeded, "TagValue", 100));
				}
...
",
		...
		public async Task<RepositoryResponse<bool>> UpdateTagTypeAsync(Guid uid, string value)
		{
			...
			if (!value.IsValidForTag(out bool isMaxLengthExceeded))
			{
				if (isMaxLengthExceeded)
				{
					new RepositoryResponse<bool>(400, string.Format(Error.MaxLengthExceeded, "TagValue", 100));
				}
