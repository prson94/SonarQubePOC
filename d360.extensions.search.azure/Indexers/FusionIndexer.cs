...
namespace d360.extensions.search.azure.Indexers
{
    public class FusionIndexer : IIndexer
    {
        ...
        public FusionIndexer(CompanyContext context, ISearchSource searchSource)
        {
            Context = context;
        ...
        public void Build()
        {
            ...
            try
            {
                fusions = Context.FusionTypeConfigurations.Include(i => i.FusionType).ToList();
                fields = Context.FieldWithRelations.Where(i => i.ObjectType == sType).ToList();
            ...
            try
            {
                leaves = Context.LeafFusionAttributes.ToList();
