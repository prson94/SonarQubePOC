...
namespace d360.model.DataAccessLayer
{
    public class ThemeRepository : BaseRepository, IThemeRepository
    {
        ...
        void addChangeLog(Theme current, string action, Theme previous = null)
        {
            switch (action)
            {
                case "C":
...
where   T.AuditID = @ID and T.[Version] = 0", new { audit.ID });
        ...
        void addStorageFile(Guid uid, string fileSuffix, byte[] content, string extension)
        {
        ...
        void deleteStorageFile(Guid uid, string fileSuffix, string extension)
        {
            var path = $"{CompanyContext.CurrentCompanyID}/{uid}_{fileSuffix}{extension}";
