...
//outer apply (select top 1 AssetID from ResponsibilityDetail where ( (AssetID = A.ID) or (AssetID = 0 and AssetTypeID = A.AssetTypeID) ) and ResourceID = @r and (PermissionsBitMask & {(int)Permission.ModifyAsset}) = {(int)Permission.ModifyAsset}) S_E 
//outer apply (select top 1 AssetID from ResponsibilityDetail where ( (AssetID = A.ID) or (AssetID = 0 and AssetTypeID = A.AssetTypeID) ) and ResourceID = @r and (PermissionsBitMask & {(int)Permission.DeleteAsset}) = {(int)Permission.DeleteAsset}) S_D";
//            }
...
from	(
		...
		from	(
                ...
                where   not exists (select 1 from ResponsibilityDetail where PermissionsBitMask & {(int)Permission.ReadAsset} = 0 and ResourceID = @r and ( (AssetID = A.ID) or (AssetID = 0 and AssetTypeID = A.AssetTypeID) ) )
                        and not exists (select 1 from ResponsibilityDetail where PermissionsBitMask & {(int)Permission.ReadAsset} = 0 and ResourceID = @r and ( (AssetID = A.ID) or (AssetID = 0 and AssetTypeID = A.AssetTypeID) ) )
                        {filterWhereString}
				) A
		pivot	(
