...
",
        ...
        public static void BulkExecutionAssetLoad(this SqlConnection cnn, int currentResourceID, List<IntegrationExecutionAsset> assets, string targetJsonColumn = "RawObject")
        {
            ...
            using (var trans = cnn.BeginTransaction())
            {
                try
                {
                    cnn.Execute("DROP TABLE IF EXISTS #IntegrationAssetTable", transaction: trans);
                    ...
                    cnn.Execute(@"CREATE CLUSTERED INDEX IX_TempIntegrationAssetTable ON #IntegrationAssetTable ( ExecutionID ASC, SynchedAssetTypeID, SourceID ASC )", transaction: trans);
