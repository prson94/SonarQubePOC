...
namespace d360.model
{
	public partial class CompanyContext : BaseContext
	{
		...
		public Issue AssignActivityWorkflowToNewObject(WorkflowEventRegistration reg, int itemId, int workflowId, int objectId, string @object)
		{
			...
			if (item == null)
			{
				throw new Exception("invalid workflow item");
			}
			...
			if (orgIssue == null)
			{
				throw new Exception("invalid workflow issue");
			}
			...
			if (obj == null)
			{
				throw new Exception("Unable to find object details of object to reassign to");
			}
		...
		public bool ExecuteTimerSteps()
		{
			...
			foreach (dynamic transition in res)
			{
				...
				if (item == null)
				{
					throw new Exception("ERROR - CANNOT FIND THE WORKFLOW TRANSITION INSTANCE THAT WE NEED TO MARK AS COMPLETED");
				}
		...
		public async Task EvaluateWorkflowTransition(long versionStepTransitionID, long itemID, EventObjectInfo objectInfo)
		{
			...
			if (transition == null)
			{
				throw new Exception("ERROR - UNABLE TO LOCATE THE SPECIFIED WORKFLOW TRANSITION STEP");
			}
			...
			switch (transition.TransitionType)
			{
				...
				case TransitionType.Condition:
					...
					if (item == null)
					{
						throw new Exception("ERROR UNABLE TO GET THE DETAILS FOR THIS WORKFLOW INSTANCE.");
					}
			...
			if (transitionPassed)
			{
				...
				{
					throw new Exception("ERROR - CANNOT FIND ITEM FROM STEP");
				}
		...
		public async Task ExecuteStep(long itemStepID, long itemID, EventInfo eventInfo)
		{
			...
			else if (stepType == StepType.Finish || stepType == StepType.Terminate)
			{
				...
				if (item == null)
				{
					throw new Exception("ERROR - CANNOT FIND THE WORKFLOW INSTANCE THAT WE NEED TO MARK AS COMPLETED");
				}
		...
		private async Task<bool> SendWorkflowEmail(WorkflowItemStep item, EventInfo eventInfo, WorkflowItemStepSettingModel settings)
		{
			...
			{
				throw new Exception("INVALID EMAIL CONFIGURATION FOR SPECIFIED STEP.");
			}
