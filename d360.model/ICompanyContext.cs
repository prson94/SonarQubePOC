...
namespace d360.model
{
    public interface ICompanyContext : IBaseContext
    {
       ...
       DbSet<FusionSchedule> FusionSchedules { get; set; }
        ...
        List<RelationshipTypeResult> ImportRelationshipTypes(ApiExecution execution, IEnumerable<RelationshipTypeInsert> import,  int timeout = 3600, int mergeBlockSize = 500);
        List<RelationshipTypeResult> ImportRelationshipTypes(ApiExecution execution, IEnumerable<RelationshipTypeUpdate> import, int timeout = 3600, int mergeBlockSize = 500);
