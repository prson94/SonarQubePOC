...
namespace igx.functions.consumption
{
    public class WorkflowSubscriber
    {
        ...
        public async Task Run([ServiceBusTrigger("%EventBusTopicName%", "Workflow")]Message brokeredMessage, TextWriter log, ExecutionContext context)
        {
            ...
            company = JobDbContextCreator.CreateCompanyContext(companyId, info.ResourceID, info.DomainPrefix, true, queueSource, null, CoreFunction.GetConnectionString("CommunityContext"), CoreFunction.GetConfigValueByKey<string>(constants.MAIL_API_KEY));
