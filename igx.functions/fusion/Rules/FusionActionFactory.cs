...
namespace igx.functions.fusion.Rules
{
    ...
    {
        internal static IFusionRuleAction CreateAction(FusionRuleType action, FusionRuleStepModel step, TraceWriter log, int companyId, FusionRule rule)
        {
            switch (action)
            {
            ...
            log.Error($"Company ID[{companyId}] Rule ID[{rule.ID}] Step ID[{step.ID}] Action[{step.Action}] Not Supported...");

            return null;
        }

        private static IFusionRuleAction CreateUpdateAction(FusionRuleStepModel step, TraceWriter log, int companyId, FusionRule rule)
        {
            return new FusionRuleUpdate(step, log, companyId, rule);
        }

        private static IFusionRuleAction CreateFindRelation(FusionRuleStepModel step, TraceWriter log, int companyId, FusionRule rule)
        {
            return new FusionRuleFindRelation(step, log, companyId, rule);
        }

        internal static IFusionRuleAction CreateRelateAction(FusionRuleStepModel step, TraceWriter log, int companyId, FusionRule rule)
        {
            return new FusionRuleRelate(step, log, companyId, rule);
        }

        internal static IFusionRuleAction CreateFindAction(FusionRuleStepModel step, TraceWriter log, int companyId, FusionRule rule)
        {
            if (!step.Settings.ContainsKey("ObjectSearch"))
            {
                log.Error($"Company ID[{companyId}] Rule ID[{rule.ID}] Step ID[{step.ID}] Action[{step.Action}] Doesnt contain find search type.");
            ...
            switch (searchType)
            {
                ...
                default:
                    log.Error($"Company ID[{companyId}] Rule ID[{rule.ID}] Step ID[{step.ID}] Find Action doesnt contain a valid search type.");
                    return null;
            }            
        }
        

        internal static IFusionRuleAction CreatePromotionAction(FusionRuleStepModel step, TraceWriter log, int companyId, FusionRule rule)
        {
            if (!step.Settings.ContainsKey("Object"))
            {
                log.Error($"Company ID[{companyId}] Rule ID[{rule.ID}] Step ID[{step.ID}] Action[{step.Action}] Doesnt contain object setting.");
            ...
            {
                log.Error($"Company ID[{companyId}] Rule ID[{rule.ID}] Step ID[{step.ID}] Action[{step.Action}] Doesnt contain object id setting.");
            ...
            log.Info($"Company ID[{companyId}] Rule ID[{rule.ID}] Step ID[{step.ID}] Action[{step.Action}] Target[{promoteToObject}] Target ID[{promoteToObjectID}]");
            ...
            log.Error($"INVALID PROMOTION TARGET - Company ID[{companyId}] Rule ID[{rule.ID}] Step ID[{step.ID}] Action[{step.Action}] Target[{promoteToObject}] Target ID[{promoteToObjectID}]");
