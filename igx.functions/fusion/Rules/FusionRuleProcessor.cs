    ...
    public static class FusionRuleProcessor
    {
        ...
        public static async void Process(int companyId, TraceWriter log)
        {
            ...
            using (var company = CompanyConnectionUtils.GetCompanyConnection(companyId))
            {
                ...
                log.Info($"Company ID[{companyId}] Fusion rules were last run {lastPromotionRun}");
                ...
                if (!fusionRulesToRun.Any())
                {
                    ...
                    log.Info($"Company ID[{companyId}] has no fusion rules to run.");
                ...
                {
                    log.Error($"Company ID[{companyId}] fusion rules contain no step data.  This is not valid cannot run fusion rules with no steps.");
                ...
                foreach (var rule in fusionRulesToRun)
                {
                    ...
                    if (!items.Any())
                    {
                        log.Info($"Company ID[{companyId}] Rule ID[{rule.ID}] contains no items skipping rule.");

                        continue;
                    }

                    log.Info($"Company ID[{companyId}] Rule ID[{rule.ID}] Rule will run for {items.Count} items");
                    ...
                    if(!steps.Any())
                    {
                        log.Warning($"Company ID[{companyId}] Rule ID[{rule.ID}] Rule contains no steps");

                        continue;
                    }

                    log.Info($"Company ID[{companyId}] Rule ID[{rule.ID}] Rule contains [{steps.Count()}] steps");
                    
                    foreach (var step in steps)
                    {
                        log.Info($"Company ID[{companyId}] Rule ID[{rule.ID}] Step ID[{step.ID}] Action[{step.Action}] Starting...");
                        ...
                        log.Info($"Company ID[{companyId}] Rule ID[{rule.ID}] Step ID[{step.ID}] Action[{step.Action}] Completed...");
                    }
                }
        ...
        private static async Task CreateFusionRuleFieldsTempTable(FusionRule rule, List<int> items, SqlConnection company, int companyId, TraceWriter log)
        {
            using (var transaction = company.BeginTransaction())
            {
                ...
                log.Info($"Company ID[{companyId}] Rule ID[{rule.ID}] Loading field values");
        ...
        private static async Task<IEnumerable<FusionRuleStepModel>> GetFusionRuleSteps(IEnumerable<FusionRule> fusionRulesToRun, SqlConnection company, int companyId, TraceWriter log)
        {            
        ...
        private static async Task<List<int>> GetRuleItems(int ruleId, SqlConnection company, int companyId, TraceWriter log)
        {
        ...
        /// <returns></returns>
        private static async Task<IEnumerable<d360.core.entities.FusionRule>> GetFusionRulesToProcess(SqlConnection company, int companyId, TraceWriter log)
        {
		...
		union
            ...
            return await company.QueryAsync<d360.core.entities.FusionRule>(sql, new { lastRun = lastPromotionRun });
        ...
        /// <param name="company"></param>
        /// <returns></returns>
        private static async Task<bool> HasPendingFusionRules(SqlConnection company, int companyId, TraceWriter log)
        {
            ...
            if(res == 1)
            {
                log.Warning($"Company ID [{companyId}] has outstanding fusion jobs.  Will not try to run while they are pending.");
