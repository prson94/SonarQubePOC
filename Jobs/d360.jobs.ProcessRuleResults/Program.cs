              ...
              companies.ForEach(companyID =>
              {
                  try
                  {
                      using (var context = GetCompanyConnection(companyID))
                      {
                          ...
                          Console.WriteLine("BEGIN: Resolving rule result qualifiers [company id: {0}]", companyID);
...
from	RuleResultQualifier Q
		...
		outer apply (
					...
					from	Taxonomy O
                          ...
                          Console.WriteLine("END: Resolving rule result qualifiers [company id: {0}]", companyID);

                          #endregion

                          #region Resolving rule result qualifiers

                          Console.WriteLine("BEGIN: Send resolved rule results to event notification [company id: {0}]", companyID);
	...
	from	RuleResultQualifier T
			inner join #tbl S on S.RuleResultID = T.RuleResultID and S.RuleResultQualifierTypeID = T.RuleResultQualifierTypeID");
                          ...
                          Console.WriteLine("END: Send resolved rule results to event notification [company id: {0}]", companyID);
