...
namespace igx.functions.consumption
{
	public class DisplayValueUpdateProcessor: BaseWebJob
    {
		...
		public DisplayValueUpdateProcessor(IConfiguration config, ICommunity community, ICachingProvider cache, IMailProvider mail, IQueueSource queue): base(community, config)
		{
			Cache = cache;
			Mail = mail;
			Queue = queue;
				...
				try
				{
					...
					using (var companyConnection = new SqlConnection(connectionString))
					{
						...
						else if (updateInfo.RebuildAll)
						{
							...
							{
								await workspace.UpsertRebuildStatusAsync(CompanyRebuildJobToken.DisplayValues, CompanyRebuildJobStatusState.Inactive,
