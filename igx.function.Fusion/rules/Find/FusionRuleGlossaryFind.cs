...
namespace igx.function.Fusion.rules
{
    internal class FusionRuleGlossaryFind : FusionFindActionBase, IFusionRuleAction
    {
        ...
        public FusionRuleGlossaryFind(FusionRuleStepModel step, TraceWriter log, int companyId, FusionRule rule)
        {
            Step = step;
            Log = log;
            CompanyId = companyId;
        ...
        private async Task FindArtifact(List<int> itemsToPromote, SqlConnection company)
        {
            ...
            {
                Log.Error($"Company ID[{CompanyId}] Rule ID[{Rule.ID}] Step ID[{Step.ID}] Action Find Glossary No target field to match on specified.");
