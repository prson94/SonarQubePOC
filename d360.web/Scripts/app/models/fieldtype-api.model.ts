...
export class FieldTypeHelper {
	...
	public static isFieldForOperator(field: FieldType): boolean {
		const allowedFieldTypes = ['boolean', 'date', 'datetime', 'decimal', 'html', 'lookup', 'number', 'text'];
	...
	public static isFieldForOperatorAdvancedFilters(field: FieldType): boolean {
		const allowedFieldTypes = ['boolean', 'date', 'datetime', 'decimal', 'html', 'lookup', 'number', 'text', 'link', 'tag', 'score', 'path', 'computedrelationshipfield', 'json', 'relationship', 'counter'];
		return allowedFieldTypes.some((x) => x === this.getFieldType(field).toLowerCase());
	}
}