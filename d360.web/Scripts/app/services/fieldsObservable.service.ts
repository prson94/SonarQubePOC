...
export class FieldsObservableService extends BaseObservableService implements IFieldsService {
	...
	getFieldTypeEditor(name: string, assetTypeUid: string, actionTypeUid: string, relationshipTypeUid: string): Observable<FieldTypeAPIModelField> {
		let url = "";
		if (assetTypeUid) { url = `AssetTypeUid=${assetTypeUid}`; }
		if (actionTypeUid) { url = `ActionTypeUid=${actionTypeUid}`; }
		if (relationshipTypeUid) { url = `RelationshipTypeUid=${relationshipTypeUid}`; }
	...
	getRelationObjectFields(assetTypeUid: string, actionTypeUid: string, relationshipTypeUid: string, intersectTypeUid: string): Observable<SelectItem[]> {
		let url = "";
		if (assetTypeUid) { url = `intersectTypeUid=${intersectTypeUid}&AssetTypeUid=${assetTypeUid}`; }
		if (actionTypeUid) { url = `intersectTypeUid=${intersectTypeUid}&ActionTypeUid=${actionTypeUid}`; }
		if (relationshipTypeUid) { url = `intersectTypeUid=${intersectTypeUid}&RelationshipTypeUid=${relationshipTypeUid}`; }
	...
	getReferenceTypeHierarchyFields(uid: string, assetTypeUid: string, actionTypeUid: string, relationshipTypeUid: string): Observable<SelectItem[]> {
		let url = "";
		if (assetTypeUid) { url = `uid=${uid}&AssetTypeUid=${assetTypeUid}`; }
		if (actionTypeUid) { url = `uid=${uid}&ActionTypeUid=${actionTypeUid}`; }
		if (relationshipTypeUid) { url = `uid=${uid}&RelationshipTypeUid=${relationshipTypeUid}`; }
	...
	getTypeaheadFilteredByPredicateItems(e: Observable<any>, objectType: string, id: number): Observable<EditorDropDownItem[]> {
		return e.pipe(
			...
			switchMap((e) => {
				...
				if (e.value != null) { uri += `&value=${e.value}`; }
