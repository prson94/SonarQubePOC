...
export class AssetTypeService extends BaseObservableService {
    ...
    public getAssetTypes(params: any)
        ...
        var qString = '';
        if (params) {
            qString = Object.keys(params).filter((key) => params[key]).map((key) => key + '=' + params[key]).join('&');
            if (qString) {
                qString = '?' + qString;
            }
        }
    ...
    GetAssetTypeByUid(uid: string): Observable<AssetTypeApiModel & ErrorResponse> {
        return this
            .http
			.get(`api/v2/assets/types?assetTypeUid=${uid}&includeSynonymAllocation=true&includeUpdatedAndCreatedFields=true&includedashboardflag=true&includeCustomExportTemplatesFlag=true&IncludeHasV2Workflows=true&includeId=true`)
            .pipe(
