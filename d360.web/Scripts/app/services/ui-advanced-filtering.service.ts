...
export class UiAdvancedFiltering {
	...
	filterByAndLogic(dataToFilter: ReadonlyArray<any>, filters: Filters): FilteredData {
		...
		const filterOptions = {
			...
			[OperatorString.IsTrue]: (filterOption: AdvancedFilterFieldCondition) => {
				filtredData = filtredData.filter((elementToFilter: object) => {
					return this.isGivenValueEqualToSearchValue(elementToFilter[filterOption.field], 'true', filterOption);
				});
			},
			[OperatorString.IsFalse]: (filterOption: AdvancedFilterFieldCondition) => {
				filtredData = filtredData.filter((elementToFilter: object) => {
					return this.isGivenValueEqualToSearchValue(elementToFilter[filterOption.field], 'false', filterOption);
				});
			}
		};
