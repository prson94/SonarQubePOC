    ...
    template: `
        ...
        <div *ngIf="!isLoading">
            ...
            <form #mappingForm="ngForm" (ngSubmit)="save()">
                <div class="row">
                    <div class="col s12">
                        <input type="checkbox" [(ngModel)]="model.Item.IsConstantValue" name="isConstant" /> Store a fixed source value?
                    </div>
                </div>
                <div class="row">
                    <div class="col s6" *ngIf="model.Item.IsConstantValue">
                        <div class="FieldName" style="display:block;">Source</div>
                        <input type="text" [(ngModel)]="model.Item.ConstantValue" style="width:95%" name="constant" required />
                    </div>
                    <div class="col s6" *ngIf="!model.Item.IsConstantValue">
                        <div class="FieldName" style="display:block;">Source</div>
                ...
                <div class="row">
                    <div class="col s12" style="padding-top:10px">
                        <button pButton type="submit" label="Save" [disabled]="isLoading || !mappingForm.form.valid"></button>
...
export class FusionRuleStepMappingEditorComponent extends BaseComponent implements OnInit {
    ...
    load() {
        ...
        if (this.fusionRuleStepMapping == null) {
            this.mode = "Add";
            this.fusionService.getAddFusionRuleStepMapping(this.fusionRuleStep.ID)
                .then(r => {
                    this.model = r;
            ...
            this.mode = "Edit";
            this.fusionService.getEditFusionRuleStepMapping(this.fusionRuleStepMapping.ID)
                .then(r => {
                    this.model = r;
