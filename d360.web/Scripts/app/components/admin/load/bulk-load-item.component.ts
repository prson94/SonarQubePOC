        ...
        <div *ngIf="!isLoading">
            <header>
                ...
                <d3s-tile-actions [hasFilterMode]="true"
                                  ...
                                  (exportErrorsClick)="exportErrors()"
                                  [hasExportOriginal]="true"
                                  (exportOriginalClick)="exportOriginal()"></d3s-tile-actions>
            </header>
            <input type="text"
            ...
            <p-table #dt
                ...
                <ng-template pTemplate="header">
                    <tr>
                        ...
                        <th *ngFor="let column of columns"
                            [pSortableColumn]="column.datafield"
                            style="width: 250px">
                            {{column.text}}
                        ...
                        <th [pSortableColumn]="'StatusMessage'"
                            style="width: 250px">
                            Message
                    ...
                    <tr [hidden]="showSimpleFilter">
                        ...
                        <th>
                            <d3s-column-filter [field]="'RowIndex'"
                                               [datatype]="'text'"></d3s-column-filter>
                        </th>
                        <th>
                            <d3s-column-filter [field]="'StatusMessage'"
                                               [datatype]="'text'"></d3s-column-filter>
                        </th>
                    </tr>
                </ng-template>
                <ng-template pTemplate="body"
                    ...
                    <tr [pSelectableRow]="item">
                        ...
                        <td>{{item.RowIndex}}</td>
                        <td>
                            <span *ngIf="item.StatusMessage"
                                  [innerHtml]="item.StatusMessage"></span>
                        </td>
                    </tr>
                </ng-template>
...
export class BulkLoadItemComponent extends BaseComponent implements OnChanges {
    ...
    constructor(private getLoadService: GetLoadService) {
        super();
    }
    ...
    exportErrors(): void {
        if (this.checkId()) {
            this.getLoadService.getLoadErrorsXls(this.id);
        }
    }

    exportOriginal(): void {
        if (this.checkId()) {
            this.getLoadService.getLoadOriginalXls(this.id);
        }
    }
    ...
    load(): void {
        if (this.checkId()) {
            ...
            this.getLoadService.getLoadColumns(this.id).subscribe(
                responseLoadColumns => {
                    this.columns = responseLoadColumns;

                    this.getLoadService.getLoadItems(this.id).subscribe(
                        responseLoadItems => {
