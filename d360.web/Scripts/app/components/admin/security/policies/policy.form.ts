...
import { ReadSecurityPolicy, SecurityPolicyThen, SecurityPolicyWhen } from "../../../../models/security.model";
import { BaseComponent } from "../../../shared/base.component";
import { Operator } from "../../../../models/operator.model";
import { ResponsibilityRuleTestRow, ResponsibilityTypeRelationRuleFormDataFieldType } from "../../../../models/responsibility-type.model";

@Component({
    selector: "admin-policies-form",
	templateUrl: "./policy.form.html",
    providers: [SecurityService],
})

export class PolicyForm extends BaseComponent implements OnInit {
    @Input() id: string;
	...
	model: ReadSecurityPolicy = new ReadSecurityPolicy();
    ...
    private load(): void {
        if (this.id !== "") {
            ...
            let r: ReadSecurityPolicy;
			//this.securityService.getResponsibilityTypeRelationRule(this.id)
   //             .subscribe((data) => {
			//		this.model = data;
   ...
   //                         if (!this.model.StructuredDefinition.Then.Conditions) {
			...
			//						const resources = this.responsibilityTypeService.getRelationRuleFormData(this.resourceType, 1);
			//						const groups = this.responsibilityTypeService.getRelationRuleFormData(this.groupType, 1);
   ...
   //             });
            ...
            this.model = new ReadSecurityPolicy();
    ...
    addWhen(): void {
        //const whenItem: ResponsibilityTypeRelationRuleDefinitionWhenItem = new ResponsibilityTypeRelationRuleDefinitionWhenItem();
    ...
    private loadWhenValuesForFieldType(item: SecurityPolicyWhen, event: any = null): Promise<void> {
        if (item.fieldName) {
    //        let selectedFieldType = this.whenFieldTypes.find((f) => f.fieldTypeName === item.fieldName);
    //        if (selectedFieldType) {
    //            selectedFieldType = cloneDeep(selectedFieldType);
    //            item.fieldName = selectedFieldType.fieldTypeName ?? selectedFieldType.label;
    ...
    //        }
            ...
            //const selectedIntersectType = this.whenIntersectTypes.find((f) => f.value === item.IntersectTypeID);
    ...
    parseRelationshipWhenValue(item: SecurityPolicyWhen): Promise<void> {
        //if (item.Value) {
   ...
   //         });
    ...
    mapToWhenV2(when: SecurityPolicyWhen) {
	...
	mapToWhenFieldValueForV2(when: SecurityPolicyWhen) {
  //      if (!when.IsLookup) {
  //          return when.Value;
  //      }
  ...
  //      const field = this.whenFieldTypes.find((field) => field.value === when.FieldTypeID);
	...
	mapToWhenRelationIntersectTypeUidForV2(when: SecurityPolicyWhen) {
        const intersectType = this.whenIntersectTypes.find((i) => i.uid === when.intersectTypeUid);
        return intersectType.uid;
    }

	mapToWhenRelationAssetUidForV2(when: SecurityPolicyWhen) {
        //const item = when.IntersectTypeValueOptions.find((i) => i.value === when.Value);
        //return item.assetUid;
    }

    addThenCondition(): void {
		//const thenItem: SecurityPolicyWhen = new ResponsibilityTypeRelationRuleDefinitionThenItem();
  ...
  //      this.model.StructuredDefinition.Then.Object = item.Object;
   ...
   //         });
    ...
    getAssigneeTypeUid(then: SecurityPolicyThen) {
  ...
  //      return field.assigneeTypeUid;
	...
	//mapToThenV2(then: SecurityPolicyThen): RuleThenV2 {
	//	var fieldTypes = then.Object === this.resourceType ? this.thenUserFieldTypes : this.thenGroupFieldTypes;
 //       const field = fieldTypes.find((field) => field.value === then.FieldTypeID);
 //       if (field.isLookup) {
 ...
 //   }
	...
	private loadThenValuesForFieldType(item: any, clearValue?: boolean): Promise<void> {
		...
		const selectedFieldType = fieldTypes.find((f) => f.value === item.FieldTypeID);
    ...
    private loadValuesForIntersectType(item: SecurityPolicyWhen): Promise<void> {
