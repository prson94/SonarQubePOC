...
@Component({
    ...
    template: `
        ...
        <div class="row" *ngIf="!isLoading">
            <form (ngSubmit)="onSubmit()" #FusionAttributeTypeCustomQueryForm="ngForm">
                <div class="col s12">
                    <div class="FieldName">Attribute Type</div>
                    <div>
                        <select required name="fat" [(ngModel)]="editedOverride.FusionAttributeTypeID"
                                style="width:75%">
                            <option *ngFor="let i of attributeTypes" [value]="i.ID">{{i.TextPath}}</option>
                        </select>
                    </div>
                </div>
...
export class FusionAttributeTypeCustomQueryEditorComponent extends BaseComponent implements OnInit {
    ...
    ngOnInit() {
        this.fusionService
            .getFusionAttributeTypeList(this.fusionId)
