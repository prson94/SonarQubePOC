...
export class AdminSiteMenuFolderEditorComponent extends BaseComponent implements OnChanges, OnInit {
	...
	removeFromSelectedFolderItems() {
		if (this.selectedItemsFromTarget.length > 0) {
			for (let j = 0; j < this.selectedItemsFromTarget.length; j++) {
				let x = this.availableItems.findIndex((i) => i.ObjectID === this.selectedItemsFromTarget[j].ObjectID && i.Object === this.selectedItemsFromTarget[j].Object); // eslint-disable-line detect-object-injection
				let y = this.itemsFromTarget.findIndex((i) => i.ObjectID === this.selectedItemsFromTarget[j].ObjectID && i.Object === this.selectedItemsFromTarget[j].Object); // eslint-disable-line detect-object-injection (false positive)
				if (y > -1) {
	...
	moveToTop() {
		if (this.selectedItemsFromTarget.length > 0) {
			for (let j = 0; j < this.selectedItemsFromTarget.length; j++) {
				let x = this.itemsFromTarget.findIndex((i) => i.ObjectID === this.selectedItemsFromTarget[j].ObjectID && i.Object === this.selectedItemsFromTarget[j].Object); // eslint-disable-line detect-object-injection
	...
	selectAllSelectedFolderItems($event, table: Table) {
		...
		for (let i = 0; i < table.selection.length; i++) {
			let x: number = this.itemsFromTarget.findIndex((item) => item.ObjectID === table.selection[i].ObjectID && item.Object === table.selection[i].Object); // eslint-disable-line detect-object-injection
			this.selectedItemsFromTarget.push(_.cloneDeep(this.itemsFromTarget[x])); // eslint-disable-line detect-object-injection
		}
	}
