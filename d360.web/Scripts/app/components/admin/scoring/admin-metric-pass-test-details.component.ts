...
export class AdminMetricPassTestDetailsComponent implements OnChanges {
	...
	private resultOperationAsLocaleString(rule: MetricRuleResultOperation) {
		switch (rule.toString()) {
			case 'Average': return $localize`Average`;
			case 'Minimum': return $localize`Minimum`;
			case 'Maximum': return $localize`Maximum`;
		}
	}
