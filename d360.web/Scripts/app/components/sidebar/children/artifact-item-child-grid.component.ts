...
export class ArtifactItemChildGridComponent extends BaseComponent implements OnChanges {
    ...
    getData() {
        ...
        this.assetService.getArtifactType(this.artifactTypeId).subscribe(i => {
            ...
            var params = { _pagesize: this.numberOfRows, _pagenum: this.currentPage, _subjectUid: i.uid, _filter: "ParentUid eq '" + this.parentUid + "'", _order: this.sortField, _direction: sortOrderText, _simpleFilter: this.filter, _includeParent: true, useGraphForParent: this.useGraph, useTypeLevelDefaultSorts: true };
            if (params._order == '') {
