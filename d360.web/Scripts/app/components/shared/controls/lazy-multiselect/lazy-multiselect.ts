...
@Component({
	selector: 'ig-lazyMultiSelectItem',
	template: `
        <li
...
export class LazyMultiSelectItem {
	@Input() option: any;
 ...
 */
@Component({
	selector: 'ig-lazymultiselect',
	template: `
        <div
            #container
            [ngClass]="{ 'p-multiselect p-component ig-dropdown ig-input-full': true, 'p-multiselect-open': overlayVisible, 'p-multiselect-chip': display === 'chip', 'p-focus': focus, 'p-disabled': disabled }"
            [ngStyle]="style"
            [class]="styleClass"
        ...
        >
            ...
            >
                <ng-template pTemplate="content">
                    <div [ngClass]="['p-multiselect-panel p-component ig-dropdown-overlay ig-dropdown-ellipsis-ltr']" [ngStyle]="panelStyle" [class]="panelStyleClass" (keydown)="onKeydown($event)">
...
export class LazyMultiSelect implements OnInit, AfterViewInit, AfterContentInit, AfterViewChecked, ControlValueAccessor {
	...
	/**
	 * Initial Items - the selected items, so available before having been lazy loaded.
	 */
	@Input() get initialItems(): any | undefined {
		return this._initialItems;
	}
	set initialItems(val: any | undefined) {
		if (!this.lazy) {
			console.warn('The initialItems property is only used when lazy loading.');
		}
	...
	searchLabelByValue(val: any, options: any[]): string {
		let label = null;
		if (this.lazy) {
			options = [...(options || []), ...this._lazySelectedOptions, ...this.initialItems];
		}
...
@NgModule({
	...
	exports: [LazyMultiSelect, OverlayModule, SharedModule, ScrollerModule],
	declarations: [LazyMultiSelect, LazyMultiSelectItem]
})
export class LazyMultiSelectModule { }