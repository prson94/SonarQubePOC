...
import { DropdownChangeEvent, DropdownFilterEvent, DropdownFilterOptions } from 'primeng/dropdown';
 ...
 */
@Component({
	selector: 'ig-lazydropdown',
	template: `
        <div
            #container
            [ngClass]="{ 'p-dropdown p-component ig-dropdown ig-input-full': true, 'p-disabled': disabled, 'p-dropdown-open': overlayVisible, 'p-focus': focused, 'p-dropdown-clearable': showClear && !disabled }"
        ...
        >
            ...
            >
                <ng-template pTemplate="content">
                    <div [ngClass]="'p-dropdown-panel p-component ig-dropdown-overlay ig-dropdown-ellipsis-ltr'" [ngStyle]="panelStyle" [class]="panelStyleClass">
		...
		if (event.toState === 'visible') {
			...
			if (this.options && this.options.length) {
				if (this.virtualScroll) {
					const selectedIndex = (this.selectedOption && this.getOptionValue(this.selectedOption) != null) ? this.findOptionIndex(this.getOptionValue(this.selectedOption), this.optionsToDisplay as any[]) : -1;
					if (selectedIndex !== -1) {
	...
	findOptionIndex(val: any, opts: any[]): number {
		...
		if (opts) {
			for (let i = 0; i < opts.length; i++) {
				const optnVal = this.getOptionValue(opts[i]);
				if ((val == null && optnVal == null) || ObjectUtils.equals(val, optnVal, this.dataKey)) {
					index = i;
					break;
				}
			}
	...
	findOption(val: any, opts: any[], inGroup?: boolean): SelectItem {
		if (this.group && !inGroup) {
			...
			if (this.lazy) {
				...
				if (this.selectedOption !== null && this.findOptionIndex(val, [this.selectedOption]) === 0) {
...
@NgModule({
	...
	exports: [LazyDropdown, OverlayModule, SharedModule, ScrollerModule],
	declarations: [LazyDropdown, LazyDropdownItem]
})
export class LazyDropdownModule { }