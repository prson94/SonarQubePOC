	...
	setElementPosition() {
		...
		if (this.positionEl && this.isVisible) {
			...
			if (isOverflowBottom && isOverflowRight) {
				...
				this.currentLocation = PopupMenuLocation.BottomLeft;
				if (this.location) { this.currentLocation = this.location; }
				this.openToBottomSide = true;
			}

			if (this.forceOpenBottom) {
				this.openToBottomSide = true;
				this.currentLocation = PopupMenuLocation.BottomLeft;
			}
			...
			if (isUnset || hasLocationChanged) {
				if (this.forceOpenBottom) {
					...
					if (menuHeight > maxHeight) {
						var menuUlElement = menu.getElementsByTagName("UL")[0] as HTMLElement;
						menuUlElement.style.maxHeight = maxHeight + "px";
						menuUlElement.style.overflow = "auto";
					}
				}
				setTimeout(() => {
