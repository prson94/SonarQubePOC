                ...
                <span #item class="d3s-tooltip" [ngClass]="{'d3s-tooltip-active':active}" (mouseenter)="show(item,tip)" (mouseleave)="hide()" (click)="click.emit()">
                    ...
                    <div class="tooltip-child tooltip-panel" #tip>
                        <h3 style="positon: relative"><a *ngIf="data && data.Url" [routerLink]="data?.Url">{{data?.DisplayName}}</a> 
                                <small *ngIf="data && !data.Url && data.DisplayName" style="background-color: #fff; float:left;font-size:80%;">{{data?.DisplayName}}</small>
                        ...
                        <div *ngIf="data?.UID != null"><b>GUID</b>: <span [innerHtml]="data.UID"></span></div>
