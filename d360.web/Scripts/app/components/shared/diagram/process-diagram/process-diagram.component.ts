...
export class BigObjectNodeData {
	key: string;
	prop: string;
	value: string;
}
...
export class ProcessNodeDataHelper {
	...
	public setNodeData(model: go.Model, data: go.ObjectData, formData: go.ObjectData, propertyName: string) {
		...
		if (propertyName !== 'key' && !bothEmpty) {
			...
			if (fieldValue.length > this.bigObjectCharacterThreshold) {
				...
				if (bigData) {
					...
					this.nodeDataBigObjectStorage.push({ key: formData.key, prop: propertyName, value: fieldValue });
				}
				model.set(data, propertyName, this.getValueHash(fieldValue));
			}
			else {
				//handle regular size objects (DEFAULT)
				model.set(data, propertyName, fieldValue);
			}
		}
	}
...
export class ProcessDiagramComponent extends DiagramBaseComponent implements OnInit, AfterViewChecked, OnDestroy, OnChanges {
	...
	public processDataHelper: ProcessNodeDataHelper;
	...
	public onResize(event) {
		...
		if (this.isEditMode) { this.diagramRef.nativeElement.style.height = (height - 115) + 'px'; }
		else if (this.isFullScreen) { this.diagramRef.nativeElement.style.height = (height - 40) + 'px'; }
		else { this.diagramRef.nativeElement.style.height = (height - 240) + 'px'; }
		...
		if (this.myDiagram) {
			setTimeout(() => {
				if (this.myDiagram) { this.myDiagram.redraw(); }
			}, 100);
		}
	...
	ngAfterViewChecked() {
		...
		if (this.myDiagram) {
			...
			if (this.isDiagramLoaded && this.isEditMode) { this.saveState.emit(this.isCurrentStateSaved()); }
		}
	...
	ngOnDestroy() {
		if (this.cdRef) { this.cdRef.detach(); }
		if (this.myDiagram) { this.myDiagram = null; }
	}
	...
	private isRelatedAssetsEnabled() {
		if (!this.selectedNodeData) { return false; }
	...
	load(isFromSave: boolean = false) {
		...
		this.processDataHelper = new ProcessNodeDataHelper();
			...
			.subscribe((response) => {
				...
				if (res && res.nodeDataArray && res.nodeDataArray.length > 0) {
					res.nodeDataArray.forEach((x) => {
						...
						this.processDataHelper.processBigObjects(x);
					});
				}
	...
	updateNodeFromForm(formData) {
			...
			this.myDiagram.model.commit(function (m) {
				...
				if (data) {
					for (var propertyName in formData) {
						self.processDataHelper.setNodeData(m, data, formData, propertyName);
					}
		...
		if (this.myDiagram && this.myDiagram.nodes) {
			...
			this.myDiagram.nodes.each((node) => {
				if (node && node.data && node.data['Name']) { this.nodeNames.push(node.data['Name']); }
			});
		}
	}
	...
	private getNodeRoleName(data: any) {
		try {
			var item = this.colors.find((x) => +x.ObjectID === +data.GovernanceRole);
			if (item) { return item.DisplayValue; }
		}
		catch {
			return '';
		}
				...
				try {
					this.myDiagram.model.commit(function (m) {
						badges.forEach((asset) => {
							...
							if (data) { m.set(data, 'relCount', asset.RelationshipCount.toString()); }
						});
