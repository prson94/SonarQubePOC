...
export class AssetGridComponent extends BaseComponent implements OnChanges, OnDestroy {
	...
	ngOnChanges(changes: { [propName: string]: SimpleChange }) {
		if (changes['assetTypeApiModel'] && this.assetTypeApiModel != null) {
			this.load();
		}

		//clear out the filters if the artifacttype is different
		this.stateService.resetArtifactTypeFilterIfRequired(this.assetTypeApiModel.uid);
	}
	...
	load() {
		...
		this.permissionsService.getAssetTypePermissions(this.assetTypeApiModel.uid)
			.subscribe((res) => {
	...
	getThreshold(value: string, lower: number, upper: number): string {
		...
		if (v <= lower) { return 'poor'; }
		else if (v > lower && v <= upper) { return 'average'; }
		else { return 'good'; }
	...
	export(listableOnly) {
		...
		this.assetService
			.downloadAssetsExcel(
				this.assetTypeApiModel.uid,
				this.getParams(),
				$localize`Filtered ${this.assetTypeApiModel.Name} List`,
	...
	downloadCustomExcel(option: AssetTypeExportTemplate) {
		...
		this.assetService.downloadAssetsExcel(this.assetTypeApiModel.uid, params, $localize`Filtered ${this.assetTypeApiModel.Name} List`, () => { this.gridCustomExport.setExportState(option, false); });
	}
