...
import { SiteMenuAssetType } from './site-menu-asset-type';
import { SiteMenuDeleteDialog } from './site-menu-delete-dialog';
import { SiteMenuFolderEdit } from './site-menu-folder-edit';
...
@Component({
	selector: 'site-menu-configuration',
	templateUrl: './site-menu-configuration.html',
	styleUrls: ['./site-menu-configuration.less'],
	...
	imports: [
		...
		PopupMenuModule,
		SiteMenuAssetType,
		SiteMenuDeleteDialog,
		SiteMenuFolderEdit,
		SiteModalModule,
		TableModule
	]
})
export class SiteMenuConfiguration extends AdminBaseComponent implements OnInit {
	...
	loadNavItems(preselectModel = null, preselectByName = null) {
		...
		this.siteMenuService.getSiteNavItems().subscribe((nav) => {
			...
			if (preselectModel) {
				const folder: SiteNav = preselectModel.folder;
	...
	addFolder(item: SiteNav) {
		const i = this.availableItems.findIndex((f) => f.ObjectID === item.ObjectID && f.Object === item.Object);
	...
	deleteFolderItem(item: SiteNav) {
		const i = this.folderItems.findIndex((f) => f.ID === item.ID);

		if (i > -1) {
			const item = this.folderItems[i];
			item.ParentID = this.selection.ID;
	...
	cancel() {
		...
		this.folderItems.forEach((o) => {
			const s = this.oldFolderItems.find((i) => i.ID === o.ID);
		...
		this.oldFolderItems.forEach((o) => {
			...
			const s = this.folderItems.find((i) => i.ID === o.ID);
