...
namespace d360.web.Controllers.V2
{
	...
	public class EnvironmentController : BaseV2ApiController
	{
		...
		public async Task<HttpResponseMessage> PostUsage(UsageEntry value)
		{
			...
			var result = await Usage.CreateUsageAsync(value, ipAddress);
		...
		public async Task<IHttpActionResult> GetLicensingDetails()
		{
			try
			{
				...
				var contributorSql = @"
					...
					left join [security].[Rule] R on AT.ID = R.AssetTypeID
					left join [security].[RuleAssignment] RA on AT.ID = RA.AssetTypeID
					inner join Asset A on A.AssetTypeID = AT.ID 
					left join [security].[Override] O on a.Id = O.AssetID;

					select count(distinct gr.ResourceID)
					from reporting.global_resource GR
					inner join [dbo].[ResponsibilitySummary] counts on counts.SecurityId = gr.ResourceID
					where (counts.Permissions is not null and ( counts.Permissions & @pm > 0 or counts.Permissions & @pd = @pd))
...
<pre>", DataType = "string", ParameterType = "formData", Required = false),
		...
		public async Task<IHttpActionResult> GetPowerBITokens(string reportId)
		{
			...
			using (var client = new PowerBIClient(new Uri(pbiUrl), tokenCredentials))
			{
				...
				return ResponseMessage(Request.CreateResponse(HttpStatusCode.Created, viewModel));
			}
		}
