...
namespace d360.web.Controllers
{
	...
	public class ResourcesController : BaseController
	{
		...
		private string getDynamicFieldSimpleFilter(string[] fixedColumns, SystemObjects type, int typeID, string filterExp, DynamicParameters dbArgs, List<FieldType> fields = null)
		{
			...
			var val = new DbString { Value = filterExp.Replace('*', '%').Replace('?', '_'), Length = 200 };
		...
		public JsonResult GetTooltipData(int objectID, string objectType, AssetTypeClass? assetTypeClass = null)
		{
			try
			{
				...
				if (Company.HasAssetPermission(objectType, objectID, Permission.ReadAsset))
				{
					...
					if (objectType == "Issue")
					{
						...
						issueRes.ForEach((item) =>
						{
							...
							if (item.AllowMultipleValues)
							{
								...
								{
									if (long.TryParse(iditem, out long listId))
									{
										itemIds.Add(listId);
									}
								}
					...
					else if ((objectType == "Artifact") || (objectType == "Taxonomy") || (objectType == "Policy"))
					{
					...
					if(det != null && det.Type != SystemObjects.IntersectType.ToString() && det.Type != SystemObjects.IssueType.ToString() && det.Type != SystemObjects.Tag.ToString())
					{
