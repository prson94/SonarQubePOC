...
namespace d360.web.Controllers.Services
{
	...
	public class WorkflowController : BaseApiController
	{
		...
		public HttpResponseMessage GetFilerDefinition()
		{
			...
			{
				new GridFilterColumn { text = "Asset", datafield = "Asset", filtertype = GridColumn.FILTER_TYPE_STRING, columntype = GridColumn.COLUMN_TYPE_STRING },
				new GridFilterColumn { text = "Type", datafield = "Type", filtertype = GridColumn.FILTER_TYPE_LIST, filteritems = filterValues, columntype = GridColumn.COLUMN_TYPE_COMBO },
				new GridFilterColumn { text = "Type Name", datafield = "TypeName", filtertype = GridColumn.FILTER_TYPE_STRING, columntype = GridColumn.COLUMN_TYPE_STRING },
				new GridFilterColumn { text = "Started", datafield = "StartedOn", filtertype = GridColumn.FILTER_TYPE_DATE, columntype = GridColumn.COLUMN_TYPE_STRING },
				new GridFilterColumn { text = "Completed", datafield = "CompletedOn", filtertype = GridColumn.FILTER_TYPE_DATE, columntype = GridColumn.COLUMN_TYPE_STRING },
				new GridFilterColumn { text = "Assigned To", datafield = "AssignedTo", filtertype = GridColumn.FILTER_TYPE_STRING, columntype = GridColumn.COLUMN_TYPE_STRING },
				new GridFilterColumn { text = "Initiator", datafield = "Initiator", filtertype = GridColumn.FILTER_TYPE_STRING, columntype = GridColumn.COLUMN_TYPE_STRING },
				new GridFilterColumn { text = "Status", datafield = "Status", filtertype = GridColumn.FILTER_TYPE_LIST, filteritems = new List<string> { "Complete", "Pending" }, columntype = GridColumn.COLUMN_TYPE_COMBO }
			};
		...
		public WorkflowDiagramModel GetWorkflowDiagram(int id, Guid? uid, int? version = null)
		{
			...
			nodes.ForEach(n =>
			{
				n.SettingsObject = XmlToDynamic(DeFormatMessageBodyTemplate(fieldTypes, n.Settings), false);
				n.FieldsObject = XmlToDynamic(DeFormatFormDescription(fieldTypes, n.Fields));
			});
		...
		public async Task<HttpResponseMessage> SubmitWorkflowForm(int itemId, int itemStepId, List<WorkflowFormModelField> model)
		{
			try
			{
				...
				if (obj != null)
				{
					type = (SystemObjects)Enum.Parse(typeof(SystemObjects), obj.Type);
				}
		...
		public async Task<HttpResponseMessage> GetWorkflowForm(int typeID, int itemStepID)
		{
			...
			foreach (var item in properties)
			{
				...
				if (string.IsNullOrEmpty(item.ReferenceFieldID) || !int.TryParse(item.ReferenceFieldID, out int fieldId) || item.FieldType != WorkflowFormModelFieldType.list)
				{
					continue;
				}
		...
		{
			var items = WorkflowActivityType.EmailNotification.GetList().ToList();
		...
		public HttpResponseMessage GetWorkflowTypesForObject(int objectID, string objectType)
		{
			...
			var types = Company.Query<dynamic>(sql, new { id = objectID, type = new DbString { Value = objectType, IsFixedLength = true, Length = 50, IsAnsi = true } }).ToList();
		...
		{
			List<FieldType> fieldTypes = getFieldTypes(model.Event.ObjectID, model.Event.Object, true);
		...
		private string FormatFormDescription(List<FieldType> fieldTypes, string data)
		{
		...
		private string DeFormatFormDescription(List<FieldType> fieldTypes, string data)
		{
			dynamic fields = XmlToDynamic(data);
		...
		private string FormatMessageBodyTemplate(List<FieldType> fieldTypes, string data)
		{
		...
		public async Task<HttpResponseMessage> GetWorkflowVersionStepDetail(int itemStepId)
		{
			...
			try
			{
				detail.FieldChanges = GetWorkFlowStepFieldChanges(detail);
				detail.RelationshipChange = GetWorkFlowStepRelationshipChanges(detail.Settings, detail.ItemID, detail.ObjectName);
					...
					if (detail.ActivityType == WorkflowActivityType.Form)
					{
						...
						if (detail.CompletedOn == null)
						{
							...
							else
							{
								...
								, new { detail.ItemID, detail.ItemStepID }).ToList();
							}
		...
		public HttpResponseMessage GetResources()
		{
			...
			queryParams.TryGetValue("excludedResourceId", out string excludedResourceId);
